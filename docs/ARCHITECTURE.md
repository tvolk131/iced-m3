...
- **Dialog:** `modal` is a persistent root host with an optional overlay. Background tree state is preserved when opening/closing. While open, the overlay covers the window, captures input, and receives operations instead of the background. The host also gates input as defense in depth. A synthetic window-unfocus event cancels in-flight background button gestures when needed; resulting background messages/redraw requests are discarded. Child content may expose nested native overlays, though a general nested-modal stack is outside this release.
