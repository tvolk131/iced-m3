...
- **Sizing:** dialog content scrolls within the viewport with 24px outer margins (smaller on tiny windows). `Dialog::actions(...)` adds a fixed footer, measured before the body so only the remaining height is scrollable. `max_height(...)` optionally caps the whole basic panel; short content still shrinks. Body-first traversal preserves keyboard order and focus reveal. Without a separate footer, all content scrolls together. A slim scrollbar reserves its own width so it does not obscure the right edge of fields or cards. The gallery uses a persistent wrapping-row structure with responsive child widths, so narrow layouts remain scrollable and app values survive resizing.
