...
| Dialogs | `dialog(content)`, `dialog::host`, `dialog::stack`, `full_screen_dialog` | Retained entrance/exit, nested focus restoration, scrim, Escape/outside dismissal and scrollable long content; `dialog::modal` is the immediate-removal compatibility host |
